...
export default function caesarCipher(string, factor) {
  ...
  const cipheredArray = splittedString.map((element) => {
    if (!isLetter(element)) {
      return element;
    }
    ...
    const wrappedCharCode = dealWithOutOfRange(charCodePlusFactor, min, max);
...
const dealWithOutOfRange = (charCode, min, max) => {
  if (charCode > max) {
    charCode -= 26;
